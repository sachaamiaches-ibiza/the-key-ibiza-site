...
// Catamaran data interface matching backend API
interface Catamaran {
  id: number;
  name: string;
  paxMax: number;
...
const CatamaransPage: React.FC<CatamaransPageProps> = ({ onNavigate, lang }) => {
  const [catamaransData, setCatamaransData] = useState<Catamaran[]>([]);
  ...
  // Fetch catamarans from backend
  useEffect(() => {
    const fetchCatamarans = async () => {
      try {
        const res = await fetch(`${BACKEND_URL}/catamarans`);
        const json = await res.json();
        // Handle both array and { data: [] } formats
        const catamarans = Array.isArray(json) ? json : (json.data || []);
        setCatamaransData(catamarans);
      } catch (error) {
        console.error('Error fetching catamarans:', error);
      }
      setLoading(false);
    };
    fetchCatamarans();
  ...
  const uniqueAmarres = useMemo(() => {
    const amarres = new Set(catamaransData.map(c => c.amarre).filter(Boolean));
    if (amarres.size === 0) return ['All'];
    return ['All', ...Array.from(amarres)];
  }, [catamaransData]);

  // Get unique locations from catamarans data
  const uniqueLocations = useMemo(() => {
    const locations = new Set(catamaransData.map(c => c.location).filter(Boolean));
  ...
  // Filter catamarans based on search criteria
  const filteredCatamarans = useMemo(() => {
    return catamaransData.filter(catamaran => {
      const matchPax = searchFilters.paxMax === 0 || catamaran.paxMax >= searchFilters.paxMax;
      const matchAmarre = searchFilters.amarre === 'All' || catamaran.amarre === searchFilters.amarre;
      const matchPrice = searchFilters.priceMax === 0 || catamaran.price <= searchFilters.priceMax;
      const matchLocation = searchFilters.localidad === 'All' || catamaran.location === searchFilters.localidad;

      // Meter range filter
      let matchMeters = true;
      if (searchFilters.metros !== 'All') {
        if (searchFilters.metros === '30+') {
          matchMeters = catamaran.metros >= 30;
        } else if (searchFilters.metros === '0-10') {
          matchMeters = catamaran.metros < 10;
        } else {
          const [min, max] = searchFilters.metros.split('-').map(Number);
          matchMeters = catamaran.metros >= min && catamaran.metros < max;
        }
      }

      return matchPax && matchAmarre && matchPrice && matchLocation && matchMeters;
    });
  }, [searchFilters, catamaransData]);
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="text-center mb-16 animate-slide-up">
          ...
          <p className="text-white/40 text-sm md:text-base font-light tracking-wide max-w-2xl mx-auto">
            Discover the freedom of sailing with our premium catamaran collection
          </p>
        </div>
            ...
            <div className="hidden md:block">
              ...
              <div className="flex justify-between items-center">
                <span className="text-xs text-white/40">
                  <span className="text-luxury-gold font-medium">{filteredCatamarans.length}</span>
                  <span className="mx-1">/</span>
                  <span>{catamaransData.length}</span>
                  <span className="ml-1">catamarans available</span>
                </span>
                <div className="flex gap-3">
                  <button
        ...
        {loading ? (
          <div className="text-center py-20 mb-20">
            ...
            <p className="text-white/40 text-sm">Loading catamarans...</p>
          </div>
        ) : filteredCatamarans.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-20">
            {filteredCatamarans.map(catamaran => (
              <div
                key={catamaran.id}
              ...
              >
                <div className="relative aspect-[4/3] overflow-hidden">
                  <img
                    src={catamaran.image || 'https://res.cloudinary.com/drxf80sho/image/upload/v1770384558/catamaran-placeholder.jpg'}
                    alt={catamaran.name}
                  ...
                  <div className="absolute bottom-4 left-4 right-4">
                    <h3 className="text-xl font-serif text-white mb-1">{catamaran.name}</h3>
                    <p className="text-luxury-gold text-sm">{catamaran.metros}m | {catamaran.paxMax} pax</p>
                  </div>
                </div>
                <div className="p-5 bg-luxury-slate/30">
                  <div className="flex justify-between items-center mb-4">
                    <span className="text-white/50 text-xs">{catamaran.amarre || catamaran.location}</span>
                    <span className="text-luxury-gold font-medium">€{catamaran.price.toLocaleString()}/day</span>
                  </div>
                  <button
          ...
          <div className="text-center py-20 mb-20">
            ...
            <p className="text-white/30 text-sm max-w-md mx-auto">
              Our exclusive catamaran collection is being curated. Contact us directly for charter inquiries.
            </p>
            <button
              onClick={() => onNavigate('contact')}
        ...
        <FooterSEO
          ...
          lang={lang}
          title="Premium Catamaran Charters in Ibiza & Formentera"
          description="The Key Ibiza offers an exceptional fleet of premium catamarans for private charter. Perfect for groups and families, these spacious vessels offer stability, comfort, and elegance as you explore the Balearic coastline in absolute luxury."
          links={[
            { label: "Luxury Yachts", view: 'boats-yachts' },
