...
const ContactForm: React.FC<ContactFormProps> = ({lang}) => {
    ...
    const BACKEND_URL_CONTACT = window.location.hostname === 'localhost' ? 'http://localhost:5001' : 'https://the-key-ibiza-backend.vercel.app';

    const sendEmail = async () => {
        try {
            const response = await fetch(`${BACKEND_URL_CONTACT}/contact`, {
                method: 'POST',
...
const App: React.FC = () => {
    ...
    // Owner Portal - self-contained full-screen app, no site chrome
    const isOwnerPortal = pathWithoutLang.startsWith('/owner');
    ...
    return (
        <div className="min-h-screen selection:bg-luxury-gold selection:text-white overflow-x-hidden">
            ...
            {!isWhiteLabelDomain && !isPhotoOnly && !isOwnerPortal &&(
            ...
            {!isWhiteLabelDomain && !isPhotoOnly && !isOwnerPortal &&<Navbar currentView={currentView} onNavigate={onNavigate} lang={lang}
            ...
            {disclaimerModalOpen && (
                <div className="fixed inset-0 z-[200000] flex items-center justify-center p-4">
                    <div
                        className="absolute inset-0 bg-black/90 backdrop-blur-md z-[199999]"
                        onClick={() => setDisclaimerModalOpen(false)}
                    />
                    <div
                        className="relative z-[200000] bg-gradient-to-b from-[#1a2634] to-[#0f1923] rounded-2xl p-6 md:p-8 w-full max-w-2xl max-h-[85vh] overflow-y-auto"
            ...
            {!isWhiteLabelDomain && !isPhotoOnly && !isOwnerPortal &&<AIConcierge lang={lang}/>}
            ...
            {!isWhiteLabelDomain && !isPhotoOnly && !isOwnerPortal &&(
                ...
                >
                    <svg className="w-7 h-7 text-luxury-gold group-hover:text-white transition-colors" fill="currentColor" viewBox="0 0 24 24">
